"use strict";
const _i = ['Phigros5', [1, 14, 514], 1611795955, 1652747710];
const urls = {
	zip: ["//cdn.jsdelivr.net/npm/@zip.js/zip.js/dist/zip.min.js", "//fastly.jsdelivr.net/npm/@zip.js/zip.js/dist/zip.min.js"],
	browser: ["/browser/Browser.js", "//fastly.jsdelivr.net/gh/mumuy/browser/Browser.js", "//passer-by.com/browser/Browser.js"],
...
window.onload = async function() {
	...
	await (async function() {
		...
		await Promise.all((obj => {
			...
			LevelOver1: "src/v2/LevelOver1.png",
			LevelOver3: "src/v2/LevelOver3.png",
			LevelOver4: "src/v2/LevelOver4.png",
			LevelOver5: "src/LevelOver5.png",
			Rank: "src/v2/Rank.png",
			NoImage: "src/0.png",
			mute: "src/mute.ogg",
...
function loop() {
	...
	ctx.globalAlpha = 1;
	if (document.getElementById("imageBlur").checked) ctx.drawImage(Renderer.bgImageBlur, ...adjustSize(Renderer.bgImageBlur, canvas, 1));
	else ctx.drawImage(Renderer.bgImage, ...adjustSize(Renderer.bgImage, canvas, 1));
	ctx.fillStyle = "#000";
	ctx.globalAlpha = 0.4;
	...
	// ctx.fillText(`Phigros Simulator v${_i[1].join('.')} - Code by homo, (canvas.width + canvasos.width) / 2 - lineScale * 0.1, canvas.height - lineScale * 0.2);
...
function qwqdraw2() {
	...
	const xhr = new XMLHttpRequest();
	xhr.open("get", `src/v2/LevelOver${difficulty < 0 ? 2 : difficulty}${hyperMode.checked ? "_v2" : ""}.ogg`, true);
	xhr.responseType = 'arraybuffer';
	xhr.send();
	xhr.onload = async () => {
...
function qwqdraw3(statData) {
	...
	ctxos.globalAlpha = 1;
	ctxos.drawImage(Renderer.bgImage, -1920 * tween[10](range(qwqEnd.second * 1)) + 1902, 539 - 600 * (Renderer.bgImage.height / Renderer.bgImage.width), 1200, 1200 * (Renderer.bgImage.height / Renderer.bgImage.width));
	ctxos.beginPath();
	ctxos.moveTo(-1920 * tween[10](range(qwqEnd.second * 1)) + 2143.8539007193578074, 208);
	ctxos.lineTo(-1920 * tween[10](range(qwqEnd.second * 1)) + 3034, 208);
	ctxos.lineTo(-1920 * tween[10](range(qwqEnd.second * 1)) + 2858.3860992806421925, 868);
	ctxos.lineTo(-1920 * tween[10](range(qwqEnd.second * 1)) + 1968.24, 868);
	ctxos.closePath();
	ctxos.globalCompositeOperation = "destination-in";
	ctxos.fill();
	ctxos.globalCompositeOperation = "source-over";
	ctxos.globalAlpha = 0.5;
	ctxos.drawImage(res["LevelOver1"], -1720 * tween[10](range(qwqEnd.second - 0.1)) + 2768, 160, 849, 424.5);
	ctxos.drawImage(res["LevelOver3"], -1020 * tween[10](range(qwqEnd.second * 0.9 - 0.25)) + 1944, 550, 950, 190);
	ctxos.drawImage(res["LevelOver3"], -1020 * tween[10](range(qwqEnd.second * 0.8 - 0.3)) + 1900, 715, 950, 190);
	ctxos.globalAlpha = 1;
	ctxos.drawImage(res["LevelOver4"], -1920 * tween[10](range(qwqEnd.second * 1)) + 1694, 137, 1616, 804);
	//歌名和等级
	ctxos.globalAlpha = 1;
	ctxos.restore();
	ctxos.setTransform(qwq0 / 120, 0, 0, qwq0 / 120, wlen - qwq0 * 8, hlen - qwq0 * 4.5); //?
	...
	ctxos.textBaseline = "middle";
	ctxos.textAlign = "left";
	ctxos.font = "73.5px Saira,sy";
	const dxsnm = ctxos.measureText(inputName.value || inputName.placeholder).width;
	if (dxsnm > 600) ctxos.font = `${73.5/dxsnm*600}px Saira,sy`;
	ctxos.fillText(inputName.value || inputName.placeholder, -1920 * tween[10](range(qwqEnd.second * 1)) + 2028, 802);
	ctxos.font = "30px Saira,sy";
	const dxlvl = ctxos.measureText(inputLevel.value || inputLevel.placeholder).width;
	if (dxlvl > 150) ctxos.font = `${30/dxlvl*150}px Saira,sy`;
	ctxos.fillText(inputLevel.value || inputLevel.placeholder, -1920 * tween[10](range(qwqEnd.second * 1)) + 2724, 825);
	//Rank图标
	ctxos.globalAlpha = range((qwqEnd.second - 1.3) * 3.75);
	const qwq2 = 293 + range((qwqEnd.second - 1.3) * 3.75) * 100;
	const qwq3 = 410 - tween[15](range((qwqEnd.second - 1.3) * 1.5)) * 164;
	...
	ctxos.fillStyle = "#fff";
	ctxos.font = "25px Saira,sy";
	ctxos.fillText(statData.newBestStr, -1720 * tween[10](range(qwqEnd.second - 0.1)) + 2855, 485);
	ctxos.fillStyle = "#fff";
	ctxos.textAlign = "left";
	ctxos.fillText(statData.scoreBest, -1720 * tween[10](range(qwqEnd.second - 0.1)) + 3005, 485);
	// 	ctxos.globalAlpha = range((qwqEnd.second - 1.87) * 2.50);
	ctxos.textAlign = "left";
	ctxos.fillText(statData.scoreDelta, -1720 * tween[10](range(qwqEnd.second - 0.1)) + 3140, 485);
	ctxos.globalAlpha = range((qwqEnd.second - 0.8) * 1.50);
	ctxos.textAlign = "right";
	ctxos.font = "50px Saira,sy";
	ctxos.fillText(stat.accStr, -1020 * tween[10](range(qwqEnd.second * 0.9 - 0.25)) + 2750, 630);
	ctxos.font = "26px Saira,sy";
	ctxos.fillText("Accuracy", -1020 * tween[10](range(qwqEnd.second * 0.9 - 0.25)) + 2755, 670);
	ctxos.textAlign = "left";
	ctxos.font = "50px Saira,sy";
	ctxos.fillText(stat.maxcombo, -1020 * tween[10](range(qwqEnd.second * 0.9 - 0.25)) + 2095, 630);
	ctxos.font = "26px Saira,sy";
	ctxos.fillText("Max Combo", -1020 * tween[10](range(qwqEnd.second * 0.9 - 0.25)) + 2095, 670);
	// ctxos.fillStyle = statData[4];
	//分数
	ctxos.fillStyle = "#fff";
	ctxos.textAlign = "left";
	ctxos.font = "86px Saira,sy";
	ctxos.globalAlpha = range((qwqEnd.second - 0.4) * 2.00);
	ctxos.fillText(stat.scoreStr, -1720 * tween[10](range(qwqEnd.second - 0.1)) + 2845, 420);
	//Perfect, good, bad, miss
	ctxos.font = "43px Saira,sy";
	ctxos.textAlign = "center";
	ctxos.globalAlpha = range((qwqEnd.second - 1.25) * 2.50);
	ctxos.fillText(stat.perfect, -1020 * tween[10](range(qwqEnd.second * 0.8 - 0.3)) + 2085, 800);
	ctxos.fillText(stat.good, -1020 * tween[10](range(qwqEnd.second * 0.8 - 0.3)) + 2233, 800);
	ctxos.fillText(stat.noteRank[6], -1020 * tween[10](range(qwqEnd.second * 0.8 - 0.3)) + 2340, 800);
	ctxos.fillText(stat.noteRank[2], -1020 * tween[10](range(qwqEnd.second * 0.8 - 0.3)) + 2447, 800);
	ctxos.font = "17px Saira,sy";
	ctxos.fillText("Perfect", -1020 * tween[10](range(qwqEnd.second * 0.8 - 0.3)) + 2085, 834);
	ctxos.fillText("Good", -1020 * tween[10](range(qwqEnd.second * 0.8 - 0.3)) + 2233, 834);
	ctxos.fillText("Bad", -1020 * tween[10](range(qwqEnd.second * 0.8 - 0.3)) + 2340, 834);
	ctxos.fillText("Miss", -1020 * tween[10](range(qwqEnd.second * 0.8 - 0.3)) + 2447, 834);
	ctxos.font = "26px Saira,sy";
	//Early, Late
	ctxos.textAlign = "left";
	ctxos.fillText("Early", -1020 * tween[10](range(qwqEnd.second * 0.8 - 0.3)) + 2540, 800);
	ctxos.fillText("Late", -1020 * tween[10](range(qwqEnd.second * 0.8 - 0.3)) + 2540, 826);
	ctxos.textAlign = "right";
	ctxos.fillText(stat.noteRank[7], -1020 * tween[10](range(qwqEnd.second * 0.8 - 0.3)) + 2705, 800);
	ctxos.fillText(stat.noteRank[3], -1020 * tween[10](range(qwqEnd.second * 0.8 - 0.3)) + 2705, 826);
}
