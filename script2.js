"use strict";
const _i = ['Phigros2', [1, 14, 514], 1611795955, 1652747710];
const urls = {
...
window.onload = async function() {
	...
	await (async function() {
		...
		res["Clicks"]["rgba(255,236,160,0.8823529)"] = await qwqImage(res["clickRaw"], "rgba(255,255,255,1)"); //#fce491
		res["Clicks"]["rgba(168,255,177,0.9016907)"] = await qwqImage(res["clickRaw"], "rgba(255,255,255,1)"); //#97f79d
		res["Clicks"]["rgba(180,225,255,0.9215686)"] = await qwqImage(res["clickRaw2"], "rgba(255,255,255,0.99)"); //#9ed5f3
...
function qwqdraw1(now) {
	...
	for (const i of clickEvents1) { //绘制打击特效1
		const tick = (now - i.time) / i.duration;
		ctxos.globalAlpha = 1;
		ctxos.setTransform(...imgFlip(noteScale * 5.3, 0, 0, noteScale * 5.3, i.offsetX, i.offsetY)); //缩放
